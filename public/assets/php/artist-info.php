    ...
    <div class="main"> <!-- div phần thân -->
        <div id="library"> <!--div thư viện-->
            ...
            <div id="artist"> <!--div nghệ sĩ-->
                ...
                $sql="SELECT nghesy.MaNgheSy,nghesy.TenNgheSy, nghesy.SoNguoiTheoDoi, nghesy.AnhNgheSy, baihat.TenBaiHat, baihat.AnhBaiHat
                FROM baihat join nghesy on baihat.MaNgheSy = nghesy.MaNgheSy
                WHERE nghesy.MaNgheSy=1";
                $result = mysqli_query($conn, $sql);
                $row=mysqli_fetch_assoc($result);
                ?>

                <div id="main-artist">
            <span id="avatar-artist"><img src="<?php echo $row["AnhNgheSy"] ?>"></span>
                    <div class="info-artist">
                        <div id="artist-text">Nghệ sĩ<br></div>
                        <div id="artist-name"><b> <?php echo $row["TenNgheSy"] ?> </b><br></div>
                        <div id="artist-follower"> <?php echo $row["SoNguoiTheoDoi"]  ?> người theo dõi</div>
                    </div>
                </div> 
                <div id="play">
                    ...
                    <form action="" method = "POST">
                        <button type ="submit" name = "follow" id="follow-button" >Theo dõi</button>
                    </form>
                    ...
                    if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['follow'])){
                        ...
                        $sql1="update nghesy set SoNguoiTheoDoi = SoNguoiTheoDoi + 1 where MaNgheSy = ?";
                        $stmt = mysqli_prepare($conn,$sql1);
                        if ($stmt === false) {
                            die("Lỗi chuẩn bị câu lệnh: " . mysqli_error($conn));
                        }
                        // Gắn giá trị vào các tham số trong prepared statement
                        mysqli_stmt_bind_param($stmt, "i", $row["MaNgheSy"]); // truyền dữ liệu vào
                    ...
                    <div id="songs">
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                echo '<div id="song1">';
                                echo '<span id="stt">1</span>';
                                echo '<span><img src="' . $row["AnhBaiHat"] . '" alt="Ảnh bài hát"></span>';
                                echo '<span id="namesong">' . $row["TenBaiHat"] . '<br>';
                                echo '<span id="author">' . $row["TenNgheSy"] . '</span>';
                                echo '</span>';
                                echo '<span id="date">10/12/2024</span>';
                                echo '</div>'   ;
                            }
                        }
                        ?>
            ...
            <div id="info-div"> <!--Thông tin bài hát nghệ sĩ-->
                ...
                <div id="info-body">
                    <div id="infodiv-song">
                        ...
                        <span id="info-namesong">I can do it with a broken heart<br><span id="info-authorsong">Taylor Swift</span></span>
                    </div>
                    <div id="custom-rate">
        ...
        <div background-color="black" id="fot"> <!--div chân play pause-->
            ...
            <div id="control">
                ...
                <button><svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M12.0029 28.5C13.6529 28.5 15.0029 27.15 15.0029 25.5V10.5C15.0029 8.85 13.6529 7.5 12.0029 7.5C10.3529 7.5 9.00293 8.85 9.00293 10.5V25.5C9.00293 27.15 10.3529 28.5 12.0029 28.5ZM21.0029 10.5V25.5C21.0029 27.15 22.3529 28.5 24.0029 28.5C25.6529 28.5 27.0029 27.15 27.0029 25.5V10.5C27.0029 8.85 25.6529 7.5 24.0029 7.5C22.3529 7.5 21.0029 8.85 21.0029 10.5Z" fill="black"/>
                    </svg></button>
                <button><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20.0029 18.889C20.0029 19.1836 19.879 19.4662 19.6583 19.6746C19.4377 19.8829 19.1384 20 18.8263 20C18.5143 20 18.215 19.8829 17.9943 19.6746C17.7737 19.4662 17.6497 19.1836 17.6497 18.889V11.6718L2.90209 19.7511C1.63136 20.4466 0.00292969 19.6045 0.00292969 18.2046L0.00292969 1.79713C0.00292969 0.397249 1.63136 -0.447121 2.90209 0.248375L17.6497 8.32767L17.6497 1.11274C17.6497 0.818085 17.7737 0.535492 17.9943 0.327137C18.215 0.11878 18.5143 0.00172806 18.8263 0.00172806C19.1384 0.00172806 19.4377 0.11878 19.6583 0.327137C19.879 0.535492 20.0029 0.818085 20.0029 1.11274L20.0029 18.889Z" fill="white"/>
                    </svg></button>
                <button><svg width="24" height="22" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M5.8917 2.11111L2.00293 5.44445L5.8917 9.33333" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M2.00293 5.44444H14.7772C18.6011 5.44444 21.8485 8.56688 21.9976 12.3889C22.1551 16.4275 18.818 19.8889 14.7772 19.8889H5.33537" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg></button>
                </div>
                <div id="progress">
